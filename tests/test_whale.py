...
def test_agent_only_stores_reusable_epistemic_notes(tmp_path):
    ...
    resumed = WhaleAgent.from_session(
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".whale").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".whale" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "whale/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "whale.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("whale.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = whale_pkg.build_agent(args)
...
def test_build_arg_parser_defaults_provider_to_openai(tmp_path):
    args = whale_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider == "openai"


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = whale_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = whale_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "whale.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("whale.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("whale.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            whale_pkg.build_agent(args)
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "WHALE_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "WHALE_DEEPSEEK_API_KEY=sk-project-deepseek",
                "WHALE_DEEPSEEK_MODEL=deepseek-v4-pro",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .whale/.\n"
    ...
    index_path = tmp_path / ".whale" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".whale" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".whale" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".whale" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".whale" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert WhaleAgent is not None
    assert OllamaModelClient is not None
    assert SessionStore is not None
    assert WorkspaceContext is not None
    assert Path(whale_pkg.__file__).as_posix().endswith("/whale/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(whale_pkg.main)
    assert callable(whale_pkg.build_agent)
    assert callable(whale_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "whale", "--help"],
        capture_output=True,
        text=True,
    )
